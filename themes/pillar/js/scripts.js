...
mr = (function (mr, $, window, document){
    ...
    var documentReady = function($){
        ...
        $('.input-radio').on('click', function() {
            var radio   = $(this);
            // radio.closest('form').find('.input-radio').removeClass('checked'); // original
            radio.closest('.radio-group').find('.input-radio').removeClass('checked');
    ...
    mr.forms.submit = function(e){
        ...
        var body          = $('body'),
            ...
            submitButton  = thisForm.find('button[type="submit"], input[type="submit"], .confirmar, .action-validate'),
            error         = 0,
        ...
        errorText = thisForm.attr('data-error') ? thisForm.attr('data-error') : "Preencha os campos corretamente";
        successText = thisForm.attr('data-success') ? thisForm.attr('data-success') : "Obrigado";
            ...
            if(masonryContainer.find('.masonry__item[data-masonry-filter]').length){
                ...
                masonryContainer.find('.masonry__item[data-masonry-filter]').each(function(){
                    var masonryItem  = $(this),
                        masonryItemName = masonryItem.attr('data-masonry-item-name'), // *custom code*
                    ...
                    jQuery(filtersArray).each(function(index, tag){
                        ...
                        if(!filtersList.find('[data-masonry-filter="'+tag+'"]').length){
                            // *custom code*
                            if($('.masonry--smooth-scroll').length){
                                filtersList.append('<li class="masonry__item-link" data-masonry-filter="'+tag+'"><a class="inner-link" href="#'+masonryContainer.attr("id")+'">'+masonryItemName+'</a></li>');
                            } else {
                                filtersList.append('<li data-masonry-filter="'+tag+'">'+tag+'</li>'); // *original code*
                            }                            
                        }
...
mr = (function (mr, $, window, document){
    ...
    var documentReady = function($){
        $('a:not([href^="#"]):not([href^="javascript:void(0)"]):not([href^="tel"]):not([href^="mailto"]):not([data-lightbox]):not([href=""]):not([target="_blank"])').on('click', function(){
